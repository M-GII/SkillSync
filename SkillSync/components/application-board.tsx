...
const COLUMN_CONFIG : Array<ColConfig> = [ 
    {color:"bg-cyan-500", icon:<Calendar className="w-5 h-5"/>},
    {color:"bg-purple-500",icon: <CheckCircle2 className="w-5 h-5"/>},
    {color:"bg-green-500",icon: <Mic className="w-5 h-5"/> },
    {color:"bg-yellow-500",icon: <Award className="w-5 h-5"/> },
        ...
        <Card className="min-w-[300px] flex-shrink-0 shadow-md p-0">
            <CardHeader className={`${config.color} text-white rounded-t-lg pb-3 pt-3`}>
                <div className="flex items-center justify-between">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger  render={<Button variant="ghost" className="h-6 w-6 text-white hover:bg-white/20" />}>
                            <MoreVertical className="h-4 w-4" />
                        </DropdownMenuTrigger>
                        <DropdownMenuContent  align="end">
                            <DropdownMenuItem  className="text-destructive">
                               <Trash2 className="mr-2 h-5 w-5" /> Delete Column
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
...
export default function ApplicationBoard({ board, userId }:ApplicationBoardProps) {
