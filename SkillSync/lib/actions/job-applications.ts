...
export async function createJobApplication(data:JobApplicationData){
    ...
    const jobApplication= await JobApplication.create({
        company,position,location,notes,salary,jobUrl,columnId,boardId,userId: session.user.id,tags: tags || [],description,status: "applied",order: maxOrder ? maxOrder.order + 1 : 0,
    })
...
export async function updateJobApplication(
  ...
  if (isMovingToDifferentColumn) {
    ...
    const jobsInTargetColumn = await JobApplication.find({ columnId: newColumnId, _id: { $ne: id } }).sort({ order: 1 }).lean();
    ...
    const otherJobsInColumn = await JobApplication.find({ columnId: currentColumnId, _id: { $ne: id } }).sort({ order: 1 }).lean();
